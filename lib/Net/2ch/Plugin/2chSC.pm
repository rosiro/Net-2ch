...
sub gen_conf {
    ...
    my $url = $conf->{url};
    my ($host, $bbs, $key);
    if ($url =~ m|^http://([\w\d]+)\.2ch\.sc/test/read.cgi/([^/]+)/(\d+)/|i) {
	($host, $bbs, $key) = ($1, $2, $3);
	#print "1 ".$url."\n";
    } elsif ($url =~ m|^http://([\w\d]+)\.2ch\.sc/([^/]+)/|i) {
	($host, $bbs) = ($1, $2);
	...
	bbs => $bbs,
	key => $key,
	setting => "http://$host.2ch.sc/$bbs/SETTING.TXT",
	subject => "http://$host.2ch.sc/$bbs/subject.txt",
	dat => "http://$host.2ch.sc/$bbs/dat/$key.dat",
	local_path => "$host.2ch.sc/$bbs/",
    });
    $self->config;
}
